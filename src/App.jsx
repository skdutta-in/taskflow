...
const GOOGLE_CLIENT_ID = "771961459047-96mcbgqn8u0p03rvk26pi30nhhgoum1p.apps.googleusercontent.com";
...
const DEFAULT_SETTINGS = {
  theme: "dark", hideCompletedInAll: false,
  showDueDateWarning: true, defaultPriority: "normal", sortOrder: "date-asc",
};
...
function CollapsibleSection({ label, sublabel, labelColor, count, children, T }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <button onClick={() => setOpen(o => !o)} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", background:"transparent", border:`1px solid ${T.border2}`, borderRadius: open?"12px 12px 0 0":12, padding:"10px 14px", cursor:"pointer", fontFamily:"inherit", borderBottom: open?"none":`1px solid ${T.border2}` }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.textMuted} strokeWidth="2.5" strokeLinecap="round" style={{ transform:open?"rotate(180deg)":"none", transition:"transform 0.2s" }}><polyline points="6 9 12 15 18 9"/></svg>
        </div>
      </button>
      {open && (
        <div style={{ border:`1px solid ${T.border2}`, borderTop:"none", borderRadius:"0 0 12px 12px", padding:"10px" }}>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
      {tasks.map(t => {
        ...
        return (
          <div key={t.id}>
            <button onClick={() => onToggle(t.id)} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:warn?"10px 10px 0 0":10, background:isChecked?accentBg:T.inputBg, border:`1px solid ${warn?"#FF2D5560":isChecked?accentColor+"40":T.border2}`, borderBottom:warn?"none":undefined, textAlign:"left", cursor:"pointer", fontFamily:"inherit" }}>
            ...
            {warn && <div style={{ background:"#FF2D5510", border:"1px solid #FF2D5530", borderTop:"none", borderRadius:"0 0 8px 8px", padding:"5px 12px", color:"#FF2D55", fontSize:11 }}>⚠ {warn}</div>}
          </div>
        );
      })}
    </div>
...
function SyncBadge({ status, lastSaved, T }) {
  const cfg = {
    idle:    { color: T.textMuted,  dot: T.border2,  label: lastSaved ? `Saved ${lastSaved}` : "Local only" },
    syncing: { color: "#F59E0B",    dot: "#F59E0B",  label: "Saving…" },
    ok:      { color: "#30D158",    dot: "#30D158",  label: "Synced" },
    error:   { color: "#FF2D55",    dot: "#FF2D55",  label: "Sync failed" },
  }[status] || {};
  return (
    <div style={{ display:"flex", alignItems:"center", gap:5 }}>
      <div style={{ width:6, height:6, borderRadius:"50%", background:cfg.dot, animation: status==="syncing"?"pulse 1s infinite":"none" }}/>
      <span style={{ color:cfg.color, fontSize:10, fontFamily:"'DM Mono',monospace" }}>{cfg.label}</span>
    </div>
  );
}
...
export default function App() {
  ...
  function sortTaskList(list) {
    return [...list].sort((a,b) => {
      if (settings.sortOrder==="priority") { const po={critical:0,urgent:1,high:2,normal:3,low:4}; return po[a.priority]-po[b.priority]||a.dueDate.localeCompare(b.dueDate); }
  ...
  const filteredTasks = useMemo(() => {
    if (view==="today") {
      const backlog=backlogTasks.filter(t=>!t.done), todayPending=tasks.filter(t=>t.dueDate===today&&!t.done), done=tasks.filter(t=>(t.dueDate===today||isBacklog(t,today))&&t.done);
    ...
    if (view==="week") { const po={critical:0,urgent:1,high:2,normal:3,low:4}; return sortTaskList(tasks.filter(t=>t.dueDate>=today&&t.dueDate<=weekEnd&&po[t.priority]<=2)); }
    let all=[...tasks]; if(settings.hideCompletedInAll) all=all.filter(t=>!t.done); return sortTaskList(all);
  ...
  function openNew()   { setEditTask(null); setForm({title:"",priority:settings.defaultPriority||"normal",dueDate:today,dependsOn:[],blocks:[],notes:""}); setShowForm(true); }
  function openEdit(task) { setEditTask(task); setForm({title:task.title,priority:task.priority,dueDate:task.dueDate,dependsOn:task.dependsOn||[],blocks:task.blocks||[],notes:task.notes||""}); setShowForm(true); setSelectedTask(null); }

  const depWarnings = useMemo(() => {
    if(!settings.showDueDateWarning||!form.dueDate) return {};
    const w={}; form.dependsOn.forEach(id=>{const d=tasks.find(t=>t.id===id); if(d&&d.dueDate>form.dueDate) w[id]=`Due ${d.dueDate} — after this task's due date!`;}); return w;
  }, [form.dependsOn,form.dueDate,tasks,settings.showDueDateWarning]);

  const blockWarnings = useMemo(() => {
    if(!settings.showDueDateWarning||!form.dueDate) return {};
    const w={}; form.blocks.forEach(id=>{const b=tasks.find(t=>t.id===id); if(b&&b.dueDate<form.dueDate) w[id]=`Due ${b.dueDate} — before this task's due date!`;}); return w;
  }, [form.blocks,form.dueDate,tasks,settings.showDueDateWarning]);

  function saveForm() {
    if(!form.title.trim()) return;
    if(editTask) {
      setTasks(ts=>{let u=ts.map(t=>t.id===editTask.id?{...t,...form}:t); u=u.map(t=>{if(t.id===editTask.id)return t;if(form.blocks.includes(t.id))return{...t,dependsOn:(t.dependsOn||[]).includes(editTask.id)?t.dependsOn:[...(t.dependsOn||[]),editTask.id]};return{...t,dependsOn:(t.dependsOn||[]).filter(d=>d!==editTask.id)};}); return u;});
    } else {
      const newId=nextId;
      setTasks(ts=>{let u=[...ts,{id:newId,...form,done:false}]; u=u.map(t=>{if(t.id===newId)return t;if(form.blocks.includes(t.id))return{...t,dependsOn:(t.dependsOn||[]).includes(newId)?t.dependsOn:[...(t.dependsOn||[]),newId]};return t;}); return u;});
      setNextId(n=>n+1);
    }
    setShowForm(false);
  }

  function toggleDone(id) { setTasks(ts=>ts.map(t=>{if(t.id!==id)return t;if(!t.done&&!canComplete(t,ts))return t;return{...t,done:!t.done};})); }
  function deleteTask(id) { setTasks(ts=>ts.filter(t=>t.id!==id).map(t=>({...t,dependsOn:(t.dependsOn||[]).filter(d=>d!==id),blocks:(t.blocks||[]).filter(d=>d!==id)}))); setSelectedTask(null); }
  ...
  function handleExport() { const csv=tasksToCSV(tasks),blob=new Blob([csv],{type:"text/csv"}),url=URL.createObjectURL(blob),a=document.createElement("a"); a.href=url;a.download=`taskflow_${today}.csv`;a.click();URL.revokeObjectURL(url); }
  function handleImportFile(e) {
    const file=e.target.files[0]; if(!file) return;
    const reader=new FileReader();
    reader.onload=(ev)=>{try{const imported=csvToTasks(ev.target.result);if(!imported.length){setImportError("No valid tasks found.");return;}const maxId=Math.max(...imported.map(t=>t.id),nextId-1);setTasks(imported);setNextId(maxId+1);setImportError("");setImportSuccess(`✓ Imported ${imported.length} tasks!`);setTimeout(()=>setImportSuccess(""),3000);}catch{setImportError("Failed to parse CSV.");}};
    ...
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif", background:T.bg, minHeight:"100vh", display:"flex", justifyContent:"center" }}>
      ...
      <div style={{ width:"100%", maxWidth:430, display:"flex", flexDirection:"column", minHeight:"100vh" }}>
        ...
        <div style={{ padding:"52px 24px 20px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              {/* Google Drive button */}
              <button className="btn-tap" onClick={()=>setShowGDrive(true)} title="Google Drive Sync" style={{ background:gUser?"#30D15815":T.surface, border:`1px solid ${gUser?"#30D15840":T.border2}`, borderRadius:14, width:44, height:44, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", position:"relative" }}>
                {gUser
                  ? <img src={gUser.picture} alt="" style={{ width:26, height:26, borderRadius:"50%" }} onError={e=>{e.target.style.display="none";}}/>
                  : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z" fill={T.textMuted}/><path d="M6.5 11.5L12 6l5.5 5.5" stroke={T.textMuted} strokeWidth="1.5" strokeLinecap="round" fill="none"/></svg>
                }
                {gToken && <div style={{ position:"absolute", bottom:6, right:6, width:8, height:8, borderRadius:"50%", background: syncStatus==="ok"?"#30D158":syncStatus==="syncing"?"#F59E0B":syncStatus==="error"?"#FF2D55":T.border2, border:`1.5px solid ${T.surface}` }}/>}
              </button>
              {/* Settings */}
              <button className="btn-tap" onClick={()=>setShowSettings(true)} style={{ background:T.surface, border:`1px solid ${T.border2}`, borderRadius:14, width:44, height:44, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={T.textMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
              </button>
              {/* CSV */}
              <button className="btn-tap" onClick={()=>setShowCSV(true)} style={{ background:T.surface, border:`1px solid ${T.border2}`, borderRadius:14, width:44, height:44, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={T.textMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              </button>
              {/* Add */}
          ...
          {stats.backlog>0&&view!=="today"&&(
            <div onClick={()=>setView("today")} style={{ background:"#FF2D5510", border:"1px solid #FF2D5530", borderRadius:12, padding:"10px 14px", marginBottom:16, display:"flex", alignItems:"center", gap:10, cursor:"pointer" }}>
              <span style={{ fontSize:16 }}>⚠️</span><span style={{ color:"#FF2D55", fontSize:13 }}><strong>{stats.backlog} overdue task{stats.backlog>1?"s":""}</strong> — tap to view</span>
            </div>
          )}

          {/* Tabs 2×2 */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
            {[{key:"today",label:"Today",badge:stats.backlog},{key:"tomorrow",label:"Tomorrow",badge:0},{key:"week",label:"This Week ⭐",badge:0},{key:"all",label:"All Tasks",badge:0}].map(tab=>(
          ...
          {filteredTasks.length===0?(
            ...
            <>
              ...
              {filteredTasks.map((task,idx)=>{
                const p=getPriorityObj(task.priority), canDo=canComplete(task,tasks), deps=(task.dependsOn||[]).map(taskById).filter(Boolean), blocked=tasks.filter(t=>(t.dependsOn||[]).includes(task.id)), backlog=isBacklog(task,today), prev=filteredTasks[idx-1], showTodayDivider=view==="today"&&idx>0&&prev&&isBacklog(prev,today)&&!isBacklog(task,today)&&!task.done;
                return (
                  <div key={task.id}>
                    {showTodayDivider&&(<div style={{ display:"flex", alignItems:"center", gap:10, margin:"10px 0" }}><div style={{ flex:1, height:1, background:T.border2 }}/><span style={{ color:T.textMuted, fontSize:10, fontFamily:"'DM Mono',monospace", letterSpacing:1 }}>TODAY</span><div style={{ flex:1, height:1, background:T.border2 }}/></div>)}
                      ...
                      <div style={{ display:"flex", alignItems:"flex-start", gap:12, paddingLeft:6 }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:6, flexWrap:"wrap" }}>
                            <span style={{ color:backlog&&!task.done?"#FF2D55":T.textMuted, fontSize:11, fontFamily:"'DM Mono',monospace" }}>{task.dueDate===today?"Today":task.dueDate===tomorrow?"Tomorrow":task.dueDate}{backlog&&!task.done?" ⚠ Overdue":""}</span>
        ...
        <div style={{ padding:"16px 24px", borderTop:`1px solid ${T.border}`, background:T.surface, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <span style={{ color:T.textMuted, fontSize:11, fontFamily:"'DM Mono',monospace" }}>{stats.pending} pending</span>
          <a href="https://skdutta.in" target="_blank" rel="noopener noreferrer" style={{ color:T.textMuted, fontSize:11, textDecoration:"none", fontFamily:"'DM Mono',monospace" }}>skdutta.in</a>
        </div>

        {/* ── Google Drive Modal ── */}
        {showGDrive&&(
          <div className="fade-in" onClick={()=>setShowGDrive(false)} style={{ position:"fixed", inset:0, background:T.modalBg, zIndex:50, display:"flex", alignItems:"flex-end" }}>
            <div className="slide-up" onClick={e=>e.stopPropagation()} style={modalSheet}>
              ...
              {!gUser?(
                <>
                  <div style={{ ...sectionCard, textAlign:"center", padding:"24px 16px" }}>
                    <div style={{ fontSize:40, marginBottom:12 }}>🔐</div>
                    <div style={{ color:T.text, fontSize:15, fontWeight:600, marginBottom:8 }}>Sign in to enable sync</div>
                    <div style={{ color:T.textMuted, fontSize:12, marginBottom:20, lineHeight:1.5 }}>Your tasks will be saved privately in your own Google Drive under App Data — only this app can access them.</div>
                    <button className="btn-tap" onClick={handleGoogleSignIn} style={{ width:"100%", padding:"14px", borderRadius:14, border:"none", background:"white", color:"#1C1C1E", fontWeight:600, fontSize:15, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", display:"flex", alignItems:"center", justifyContent:"center", gap:10, boxShadow:"0 2px 12px #0002" }}>
                  ...
                  <div style={{ background:"#6E5BFF15", border:"1px solid #6E5BFF30", borderRadius:12, padding:"10px 14px", color:"#6E5BFF", fontSize:12, lineHeight:1.5 }}>
                    💡 After signing in, your existing local tasks will be uploaded to Drive automatically.
                  </div>
                </>
              ):(
                <>
                  <div style={sectionCard}>
                    <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
                      <img src={gUser.picture} alt="" style={{ width:44, height:44, borderRadius:"50%", border:`2px solid #30D15840` }} onError={e=>{e.target.style.display="none";}}/>
                      <div>
                        <div style={{ color:T.text, fontSize:14, fontWeight:600 }}>{gUser.name}</div>
                        <div style={{ color:T.textMuted, fontSize:12 }}>{gUser.email}</div>
                      </div>
                    ...
                    <div style={{ background:T.bg, borderRadius:10, padding:"10px 12px", border:`1px solid ${T.border}` }}>
                      <div style={{ color:T.textMuted, fontSize:11, fontFamily:"'DM Mono',monospace", marginBottom:4 }}>DRIVE FILE</div>
                      <div style={{ color:T.text, fontSize:13 }}>📄 {DRIVE_FILE_NAME}</div>
                      <div style={{ color:T.textMuted, fontSize:11, marginTop:2 }}>Stored privately in App Data folder</div>
                    </div>
                  </div>
              ...
              <button className="btn-tap" onClick={()=>setShowGDrive(false)} style={{ width:"100%", padding:"14px", borderRadius:14, border:`1px solid ${T.border2}`, background:"transparent", color:T.textMuted, fontWeight:500, fontSize:15, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginTop:12 }}>Close</button>
            </div>
          </div>
        )}
        ...
        {/* ── Task Detail Modal ── */}
        {selectedTask&&(()=>{
          const task=tasks.find(t=>t.id===selectedTask.id)||selectedTask, p=getPriorityObj(task.priority), deps=(task.dependsOn||[]).map(taskById).filter(Boolean), canDo=canComplete(task,tasks), dependents=tasks.filter(t=>(t.dependsOn||[]).includes(task.id)), backlog=isBacklog(task,today);
          return (
            <div className="fade-in" onClick={()=>setSelectedTask(null)} style={{ position:"fixed", inset:0, background:T.modalBg, zIndex:50, display:"flex", alignItems:"flex-end" }}>
              <div className="slide-up" onClick={e=>e.stopPropagation()} style={modalSheet}>
                ...
                {deps.length>0&&(<div style={{ marginBottom:16 }}><div style={labelStyle("#6E5BFF")}>Depends On</div>{deps.map(d=>(<div key={d.id} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}><div style={{ width:8, height:8, borderRadius:"50%", background:d.done?"#30D158":"#FF6B00" }}/><span style={{ color:d.done?T.textMuted:T.text, fontSize:13, textDecoration:d.done?"line-through":"none", flex:1 }}>{d.title}</span><span style={{ color:T.textMuted, fontSize:11, fontFamily:"'DM Mono',monospace" }}>{d.dueDate}</span></div>))}</div>)}
                {dependents.length>0&&(<div style={{ marginBottom:16 }}><div style={labelStyle("#FF6B00")}>It Blocks</div>{dependents.map(d=>(<div key={d.id} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}><div style={{ width:8, height:8, borderRadius:"50%", background:"#FF6B00" }}/><span style={{ color:T.text, fontSize:13, flex:1 }}>{d.title}</span><span style={{ color:T.textMuted, fontSize:11, fontFamily:"'DM Mono',monospace" }}>{d.dueDate}</span></div>))}</div>)}
        ...
        {showForm&&(
          <div className="fade-in" onClick={()=>setShowForm(false)} style={{ position:"fixed", inset:0, background:T.modalBg, zIndex:50, display:"flex", alignItems:"flex-end" }}>
            <div className="slide-up" onClick={e=>e.stopPropagation()} style={{ ...modalSheet, maxHeight:"90vh", overflowY:"auto" }}>
              ...
              <div style={{ marginBottom:16 }}><label style={labelStyle()}>Title *</label><input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="What needs to be done?" style={inputStyle}/></div>
              <div style={{ marginBottom:16 }}><label style={labelStyle()}>Priority</label><div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>{PRIORITIES.map(p=>(<button key={p.key} className="pill-btn" onClick={()=>setForm(f=>({...f,priority:p.key}))} style={{ padding:"7px 14px", borderRadius:10, fontSize:12, fontWeight:500, background:form.priority===p.key?p.bg:T.inputBg, color:form.priority===p.key?p.color:T.textMuted, border:`1px solid ${form.priority===p.key?p.color+"60":T.border2}` }}>{p.label}</button>))}</div></div>
              <div style={{ marginBottom:16 }}><label style={labelStyle()}>Due Date</label><input type="date" value={form.dueDate} onChange={e=>setForm(f=>({...f,dueDate:e.target.value}))} style={{ ...inputStyle, fontFamily:"'DM Mono',monospace", colorScheme:isDark?"dark":"light" }}/></div>
              {otherTasks.length>0&&(<>
                <CollapsibleSection label="Depends On" labelColor="#6E5BFF" count={form.dependsOn.length} sublabel="Can't start until these are done" T={T}><CheckboxList tasks={otherTasks} checked={form.dependsOn} onToggle={toggleDep} accentColor="#6E5BFF" accentBg="#6E5BFF15" T={T} warnings={depWarnings}/></CollapsibleSection>
                <CollapsibleSection label="It Blocks" labelColor="#FF6B00" count={form.blocks.length} sublabel="Can't start until THIS task is done" T={T}><CheckboxList tasks={otherTasks} checked={form.blocks} onToggle={toggleBlocks} accentColor="#FF6B00" accentBg="#FF6B0015" T={T} warnings={blockWarnings}/></CollapsibleSection>
              </>)}
              <div style={{ marginBottom:24 }}><label style={labelStyle()}>Notes</label><textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Optional notes..." rows={3} style={{ ...inputStyle, resize:"none", lineHeight:1.5, fontSize:14 }}/></div>
              <div style={{ display:"flex", gap:10 }}>
                ...
                <button className="btn-tap" onClick={saveForm} style={{ flex:2, padding:"14px", borderRadius:14, border:"none", background:form.title.trim()?"linear-gradient(135deg,#6E5BFF,#B44DFF)":T.border2, color:form.title.trim()?"white":T.textMuted, fontWeight:600, fontSize:15, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", boxShadow:form.title.trim()?"0 4px 20px #6E5BFF30":"none" }}>{editTask?"Save Changes":"Create Task"}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showSettings&&(
          <div className="fade-in" onClick={()=>setShowSettings(false)} style={{ position:"fixed", inset:0, background:T.modalBg, zIndex:50, display:"flex", alignItems:"flex-end" }}>
            <div className="slide-up" onClick={e=>e.stopPropagation()} style={{ ...modalSheet, maxHeight:"90vh", overflowY:"auto" }}>
              ...
              <div style={sectionCard}><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:4 }}>🎨 Theme</div><div style={{ color:T.textMuted, fontSize:12, marginBottom:12 }}>Switch between dark and light mode</div><div style={{ display:"flex", gap:10 }}>{["dark","light"].map(th=>(<button key={th} className="pill-btn" onClick={()=>updateSetting("theme",th)} style={{ flex:1, padding:"10px", borderRadius:12, fontSize:13, fontWeight:500, background:settings.theme===th?"linear-gradient(135deg,#6E5BFF,#B44DFF)":T.inputBg, color:settings.theme===th?"white":T.textMuted, border:`1px solid ${settings.theme===th?"transparent":T.border2}` }}>{th==="dark"?"🌙 Dark":"☀️ Light"}</button>))}</div></div>
              <div style={sectionCard}><div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}><div><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:2 }}>👁 Hide Completed Tasks</div><div style={{ color:T.textMuted, fontSize:12 }}>In "All Tasks" tab only</div></div><button className="toggle-track" onClick={()=>updateSetting("hideCompletedInAll",!settings.hideCompletedInAll)} style={{ width:48, height:28, borderRadius:14, border:"none", cursor:"pointer", background:settings.hideCompletedInAll?"#6E5BFF":"#2a2a35", padding:3, display:"flex", alignItems:"center", justifyContent:settings.hideCompletedInAll?"flex-end":"flex-start" }}><div style={{ width:22, height:22, borderRadius:11, background:"white", boxShadow:"0 1px 4px #0004" }}/></button></div></div>
              <div style={sectionCard}><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:4 }}>🔃 Sort Order</div><div style={{ color:T.textMuted, fontSize:12, marginBottom:12 }}>How tasks are ordered in each view</div><div style={{ display:"flex", flexDirection:"column", gap:8 }}>{[{key:"date-asc",label:"📅 Date — Earliest first"},{key:"date-desc",label:"📅 Date — Latest first"},{key:"priority",label:"🔴 Priority — Critical first"}].map(opt=>(<button key={opt.key} className="pill-btn" onClick={()=>updateSetting("sortOrder",opt.key)} style={{ padding:"10px 14px", borderRadius:10, fontSize:13, textAlign:"left", background:settings.sortOrder===opt.key?"#6E5BFF20":T.inputBg, color:settings.sortOrder===opt.key?"#6E5BFF":T.text, border:`1px solid ${settings.sortOrder===opt.key?"#6E5BFF50":T.border2}`, fontWeight:settings.sortOrder===opt.key?600:400 }}>{opt.label}{settings.sortOrder===opt.key&&<span style={{ float:"right", color:"#6E5BFF" }}>✓</span>}</button>))}</div></div>
              <div style={sectionCard}><div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}><div><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:2 }}>⚠️ Date Conflict Warnings</div><div style={{ color:T.textMuted, fontSize:12 }}>Warn when blocking task dates conflict</div></div><button className="toggle-track" onClick={()=>updateSetting("showDueDateWarning",!settings.showDueDateWarning)} style={{ width:48, height:28, borderRadius:14, border:"none", cursor:"pointer", background:settings.showDueDateWarning?"#6E5BFF":"#2a2a35", padding:3, display:"flex", alignItems:"center", justifyContent:settings.showDueDateWarning?"flex-end":"flex-start" }}><div style={{ width:22, height:22, borderRadius:11, background:"white", boxShadow:"0 1px 4px #0004" }}/></button></div></div>
              <div style={sectionCard}><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:4 }}>⚡ Default Priority</div><div style={{ color:T.textMuted, fontSize:12, marginBottom:12 }}>Pre-selected when creating a new task</div><div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>{PRIORITIES.map(p=>(<button key={p.key} className="pill-btn" onClick={()=>updateSetting("defaultPriority",p.key)} style={{ padding:"6px 12px", borderRadius:8, fontSize:11, fontWeight:500, background:settings.defaultPriority===p.key?p.bg:T.inputBg, color:settings.defaultPriority===p.key?p.color:T.textMuted, border:`1px solid ${settings.defaultPriority===p.key?p.color+"60":T.border2}` }}>{p.label}</button>))}</div></div>
              <div style={{ textAlign:"center", padding:"8px 0 4px", color:T.textMuted, fontSize:11, fontFamily:"'DM Mono',monospace" }}>TaskFlow v1.0 · skdutta.in</div>
              <button className="btn-tap" onClick={()=>setShowSettings(false)} style={{ width:"100%", padding:"14px", borderRadius:14, border:`1px solid ${T.border2}`, background:"transparent", color:T.textMuted, fontWeight:500, fontSize:15, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginTop:12 }}>Close</button>
            </div>
          </div>
        )}
        ...
        {showCSV&&(
          <div className="fade-in" onClick={()=>{setShowCSV(false);setImportError("");setImportSuccess("");}} style={{ position:"fixed", inset:0, background:T.modalBg, zIndex:50, display:"flex", alignItems:"flex-end" }}>
            <div className="slide-up" onClick={e=>e.stopPropagation()} style={modalSheet}>
              ...
              <div style={sectionCard}><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:4 }}>📤 Export Tasks</div><div style={{ color:T.textMuted, fontSize:12, marginBottom:14, lineHeight:1.5 }}>Download all <strong style={{ color:T.text }}>{tasks.length} tasks</strong> as a <code style={{ background:T.border, padding:"1px 6px", borderRadius:4, color:"#B44DFF", fontSize:11 }}>.csv</code> file.</div><button className="btn-tap" onClick={handleExport} style={{ width:"100%", padding:"12px", borderRadius:12, border:"none", background:"linear-gradient(135deg,#6E5BFF,#B44DFF)", color:"white", fontWeight:600, fontSize:14, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", boxShadow:"0 4px 16px #6E5BFF30" }}>Download CSV</button></div>
              <div style={sectionCard}><div style={{ color:T.text, fontSize:14, fontWeight:600, marginBottom:4 }}>📥 Import Tasks</div><div style={{ color:T.textMuted, fontSize:12, marginBottom:8, lineHeight:1.5 }}>Replace all current tasks from a CSV file.</div><div style={{ background:"#FF6B0015", border:"1px solid #FF6B0030", borderRadius:8, padding:"8px 12px", marginBottom:14, color:"#FF6B00", fontSize:11 }}>⚠ This will replace ALL current tasks. Export first!</div><input ref={fileInputRef} type="file" accept=".csv" onChange={handleImportFile}/><button className="btn-tap" onClick={()=>fileInputRef.current?.click()} style={{ width:"100%", padding:"12px", borderRadius:12, border:`1px solid ${T.border2}`, background:"transparent", color:T.text, fontWeight:600, fontSize:14, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>Choose CSV File</button></div>
